        ...
        <div class="mb-10">
            <nav class="mb-4 text-xs font-medium tracking-wide uppercase text-slate-400">
                <ol class="flex items-center gap-2">
                    <li>
                        <a href="{{ route('customer.dashboard') }}" class="transition-colors hover:text-indigo-600">Account</a>
                    </li>
                    <li>
                    ...
                    <li>
                        <a href="{{ route('customer.orders') }}" class="transition-colors hover:text-indigo-600">Orders</a>
                    </li>
                    <li>
                        <svg class="h-3 w-3 stroke-current" fill="none" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M9 5l7 7-7 7"/>
            ...
            <div class="space-y-8 lg:col-span-2">
                ...
                <div class="overflow-hidden rounded-2xl border border-slate-100 bg-white shadow-sm transition-shadow hover:shadow-md">
                    ...
                    <div class="p-6">
                        <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                            <div class="space-y-1">
                                <p class="text-xs font-semibold uppercase tracking-wider text-slate-400">Order Reference</p>
                                <p class="font-mono text-base font-bold text-slate-800">{{ $order->order_number }}</p>
                            </div>
                            <div class="space-y-1">
                                <p class="text-xs font-semibold uppercase tracking-wider text-slate-400">Placed Timestamp</p>
                                <p class="text-base font-medium text-slate-800">{{ $order->created_at->format('M d, Y \a\t h:i A') }}</p>
                            </div>
                            <div class="space-y-2">
                                <p class="text-xs font-semibold uppercase tracking-wider text-slate-400">Payment Status</p>
                                <div>
                            ...
                            <div class="space-y-1">
                                <p class="text-xs font-semibold uppercase tracking-wider text-slate-400">Payment Channel</p>
                            ...
                            @if($order->tracking_number)
                                <div class="sm:col-span-2 space-y-1 rounded-xl bg-slate-50 p-4 border border-slate-100">
                                    <p class="text-xs font-bold uppercase tracking-wider text-slate-400">Tracking Identifier</p>
                                    <p class="font-mono text-base font-bold text-indigo-600 tracking-normal">{{ $order->tracking_number }}</p>
                                </div>
                            @endif
                        </div>
                    </div>
                    ...
                    <div class="divide-y divide-slate-100 p-6">
                        @foreach($order->items as $item)
                            <div class="flex flex-col gap-4 py-5 first:pt-0 last:pb-0 sm:flex-row sm:items-center">
                                <div class="h-20 w-20 flex-shrink-0 overflow-hidden rounded-xl border border-slate-100 bg-slate-50 shadow-inner">
                                    @if ($item->variant && $item->variant->image_path)
                                        <img src="{{ asset('storage/' . $item->variant->image_path) }}" alt="{{ $item->product_name }}" class="h-full w-full object-cover transition duration-300 hover:scale-105">
                                    @elseif ($item->product && $item->product->primaryImage)
                                        <img src="{{ asset('storage/' . $item->product->primaryImage->image_path) }}" alt="{{ $item->product_name }}" class="h-full w-full object-cover transition duration-300 hover:scale-105">
                                    @else
                                        <div class="flex h-full w-full items-center justify-center bg-slate-100 text-slate-400">
                                            <svg class="h-6 w-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 002-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
                                        </div>
                                    @endif
                                </div>
                                <div class="flex-1 space-y-0.5">
                                    <h3 class="font-semibold leading-snug text-slate-900 transition-colors hover:text-indigo-600">{{ $item->product_name }}</h3>
                                    @if($item->variant_name)
                                        <p class="text-xs font-medium text-slate-500">{{ $item->variant_name }}</p>
                                    @endif
                                    <div class="flex flex-wrap items-center gap-x-3 gap-y-1 pt-1 text-xs font-medium text-slate-400">
                                        ...
                                        <span>Qty: <span class="text-slate-600">{{ $item->quantity }}</span> × ${{ number_format($item->price, 2) }}</span>
                                    </div>
                                </div>
                                <div class="text-left sm:text-right">
                                    <p class="text-base font-bold text-slate-900">${{ number_format($item->subtotal, 2) }}</p>
                                </div>
                            </div>
                        @endforeach
                    </div>
                ...
                <div class="overflow-hidden rounded-2xl border border-slate-100 bg-white shadow-sm transition-shadow hover:shadow-md">
                    ...
                    <div class="p-6 text-sm leading-relaxed text-slate-600">
                        ...
                        <div class="mt-4 rounded-xl bg-slate-50/60 p-4 border border-slate-100 text-slate-700 space-y-0.5">
                            <p class="font-medium">{{ $order->shipping_address_line_1 }}</p>
                            @if($order->shipping_address_line_2)
                                <p class="text-slate-500">{{ $order->shipping_address_line_2 }}</p>
                            @endif
                            <p class="font-medium">{{ $order->shipping_city }}, {{ $order->shipping_state }} {{ $order->shipping_postal_code }}</p>
                            <p class="text-xs font-bold uppercase tracking-wider text-slate-400 pt-1">{{ $order->shipping_country }}</p>
                        </div>
                    </div>
                </div>
                                        ...
                                        <li>
                                            <div class="relative pb-8">
                                                ...
                                                <div class="relative flex items-start space-x-4">
                                                    <div class="relative">
                                                        <div class="flex h-10 w-10 items-center justify-center rounded-xl bg-indigo-50 text-indigo-600 ring-8 ring-white">
                                                    ...
                                                    <div class="min-w-0 flex-1 pt-1.5">
                                                        <div class="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
                                                            ...
                                                            <p class="text-xs font-medium text-slate-400">{{ $history->created_at->format('M d, Y • h:i A') }}</p>
                                                        </div>
                                                        @if($history->notes)
                                                            <p class="mt-2 text-sm text-slate-600 bg-slate-50 rounded-xl p-3 border border-slate-100/70">{{ $history->notes }}</p>
                                                        @endif
                                                    </div>
                                                </div>
                                            </div>
                        ...
                        <div class="p-6">
                            <div class="space-y-3.5 text-sm font-medium text-slate-600">
                                ...
                                @if($order->discount_amount > 0)
                                    <div class="flex justify-between text-emerald-600 bg-emerald-50/50 px-2 py-1 rounded-lg border border-emerald-100/50">
                                ...
                                <div class="flex justify-between">
                                    ...
                                    <span>
                                        ...
                                        @else
                                            <span class="text-xs font-extrabold tracking-wider text-emerald-600 uppercase bg-emerald-50 px-2 py-0.5 rounded border border-emerald-100">Free</span>
                                        @endif
                                    </span>
                                </div>
                            ...
                            @if($order->customer_notes)
                                <div class="mt-6 rounded-xl border border-amber-100 bg-amber-50/30 p-4">
                                    <h3 class="text-xs font-bold uppercase tracking-wider text-amber-800">Customer Addendum</h3>
                                    <p class="mt-1.5 text-sm font-medium leading-relaxed text-amber-900/80">{{ $order->customer_notes }}</p>
                                </div>
                            @endif
                        </div>
                    </div>
