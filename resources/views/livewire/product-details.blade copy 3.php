...
if ($variantImagePath) {
    ...
    <div class="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden mb-12"
        ...
        <div class="border-b border-slate-100 bg-slate-50/50 px-4 sm:px-6">
            <!-- overflow-x-auto allows horizontal scrolling on tiny screens instead of breaking layouts -->
            <nav class="flex gap-4 sm:gap-6 overflow-x-auto scrollbar-none whitespace-nowrap">
                <button @click="activeTab = 'description'"
                    ...
                    class="px-1 sm:px-2 py-4 border-b-2 text-xs sm:text-sm uppercase tracking-wider transition outline-none">
                    DESCRIPTION
                </button>
        ...
        <div class="p-4 sm:p-8">
            ...
            <div x-show="activeTab === 'delevery'" x-cloak>
                <div class="rounded-2xl border border-gray-200 bg-white shadow-sm overflow-hidden">
                    ...
                    <!-- Inner Panels wrapper adjusted to p-4 standard for mobile -->
                    <div class="p-4 sm:p-6 space-y-4 sm:space-y-6">
                        <!-- Dhaka -->
                        <div class="rounded-xl border border-green-200 bg-green-50 p-4 sm:p-5">
                            <div class="flex flex-row items-center justify-between gap-2 mb-3">
                                <h3 class="text-base sm:text-lg font-bold text-green-700">
                                    ঢাকা সিটি
                                </h3>
                                <span
                                    class="rounded-full bg-green-600 px-3 sm:px-4 py-1 text-xs sm:text-sm font-semibold text-white shrink-0">
                                    ৳ ৭০
                                </span>
                            </div>
                            <ul class="space-y-2 text-sm sm:text-base text-gray-700 leading-relaxed list-disc pl-5">
                        ...
                        <div class="rounded-xl border border-blue-200 bg-blue-50 p-4 sm:p-5">
                            <div class="flex flex-row items-center justify-between gap-2 mb-3">
                                ...
                                <span
                                    class="rounded-full bg-blue-600 px-3 sm:px-4 py-1 text-xs sm:text-sm font-semibold text-white shrink-0">
                                    ৳১৫০
                                </span>
                            </div>
                            <ul class="space-y-2 text-sm sm:text-base text-gray-700 leading-relaxed list-disc pl-5">
                        ...
                        <!-- Important Notice -->
                        <div class="rounded-xl border-l-4 border-amber-500 bg-amber-50 p-4 sm:p-5">
                            <div class="flex gap-3">
                                <div class="text-xl sm:text-2xl select-none">⚠️</div>
                                <div class="min-w-0 flex-1">
                                    <h4 class="text-sm sm:text-base font-semibold text-amber-700 mb-1 sm:mb-2">
                                        গুরুত্বপূর্ণ তথ্য
                                    </h4>
                                    <ul
                                        class="space-y-1 sm:space-y-2 text-sm sm:text-base text-gray-700 leading-relaxed list-disc pl-5">
                                        <li>ঢাকার বাইরে ডেলিভারির ক্ষেত্রে শুধুমাত্র ডেলিভারি চার্জ অগ্রিম প্রদান করতে হবে।
                                        </li>
            ...
            <div x-show="activeTab === 'reviews'" x-cloak>
                        ...
                        @foreach($product->approvedReviews as $review)
                            <div class="border-b border-slate-100 pb-6 last:border-b-0 last:pb-0">
                                <div class="flex items-start gap-3 sm:gap-4">
                                    <div
                                        class="w-9 h-9 sm:w-10 sm:h-10 bg-slate-100 text-slate-800 rounded-full flex items-center justify-center font-bold text-xs sm:text-sm shrink-0 uppercase border border-slate-200">
                                        {{ substr($review->customer->name, 0, 1) }}
                                    </div>
                                    <div class="flex-1 min-w-0">
                                        <div class="flex flex-wrap items-center gap-1.5 sm:gap-2 mb-1">
                                            <h4
                                                class="font-semibold text-slate-900 text-sm sm:text-base truncate max-w-[150px] sm:max-w-none">
                                                {{ $review->customer->name }}
                                            </h4>
                                            @if($review->is_verified_purchase)
                                                <span
                                                    class="bg-emerald-50 text-emerald-700 text-[9px] sm:text-[10px] font-bold px-1.5 sm:px-2 py-0.5 rounded-full uppercase tracking-wider border border-emerald-100">
                                                    Verified
                                                </span>
                                            @endif
                                        </div>
                                        <div class="flex items-center gap-2 mb-2">
                                            <div class="flex text-amber-400 gap-0.5">
                                                @for($i = 1; $i <= 5; $i++)
                                                    <svg class="w-3 h-3 sm:w-3.5 sm:h-3.5 fill-current {{ $i <= $review->rating ? 'text-amber-400' : 'text-slate-200' }}"
                                                        viewBox="0 0 20 20">
                                                        <path
                                        ...
                                        @if($review->title)
                                            <h5 class="font-semibold text-slate-900 text-xs sm:text-sm mb-1">{{ $review->title }}</h5>
                                        @endif
                                        @if($review->comment)
                                            <p class="text-slate-600 text-xs sm:text-sm leading-relaxed">{{ $review->comment }}</p>
                                        @endif
                                    </div>
                                </div>
                            </div>
                ...
                @else
                    <div class="text-center py-10 sm:py-12">
                        <svg class="w-8 h-8 sm:w-10 sm:h-10 text-slate-300 mx-auto mb-3" fill="none" stroke="currentColor"
                            viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
                                d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" />
                        </svg>
                        <p class="text-xs sm:text-sm text-slate-400 font-medium">No reviews yet. Be the first to review this
                            product!</p>
                    </div>
                @endif
            </div>
